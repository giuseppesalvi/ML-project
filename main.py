        ...
        if FLAG_SINGLEFOLD:
            ...
            for app in applications:
                ...
                if FLAG_GAUSSIANS:
                    ...
                    generative_models = [("MVG", multivariate_gaussian_classifier2), ("Naive", naive_bayes_gaussian_classifier), ("Tied", tied_covariance_gaussian_classifier)]
                    for name, algo in generative_models:
                        DCF_min = minimum_detection_cost(algo(DTR_T, LTR_T, DTE), LTE, pi1, Cfn, Cfp)
                        print("%s: minDCF = %f" %(name,DCF_min),"\n") 
                        DCF_min = minimum_detection_cost(algo(DHTR_T, LHTR_T, DHTE), LHTE, pi1, Cfn, Cfp)
                        print("%s: minDCF = %f" %(name,DCF_min)," (noise version)\n") 
                    print("")
                ...
                if FLAG_SVM:
                    ...
                    for K in K_list:
                        for C in C_list:
                            DCF_min = minimum_detection_cost(svm_linear(DTR_T, LTR_T, DTE, K, C), LTE, pi1, Cfn, Cfp)
                            print("SVM Linear: K = %f, C = %f, minDCF = %f" %(K, C, DCF_min),"\n")
                            DCF_min = minimum_detection_cost(svm_linear(DHTR_T, LHTR_T, DHTE, K, C), LHTE, pi1, Cfn, Cfp)
                            print("SVM Linear: K = %f, C = %f, minDCF = %f" %(K, C, DCF_min)," (noise version)\n")
                    # Polynomial Kernel
                    c_list = [0,1]
                    for K in K_list:
                        for C in C_list:
                            for c in c_list:
                                DCF_min = minimum_detection_cost(svm_kernel_polynomial(DTR_T, LTR_T, DTE, K, C, d=2, c=c), LTE, pi1, Cfn, Cfp)
                                print("SVM Polynomial Kernel: K = %f, C = %f, d = 2, c = %f, minDCF = %f" %(K, C, c, DCF_min),"\n")
                                DCF_min = minimum_detection_cost(svm_kernel_polynomial(DHTR_T, LHTR_T, DHTE, K, C, d=2, c=c), LHTE, pi1, Cfn, Cfp)
                                print("SVM Polynomial Kernel: K = %f, C = %f, d = 2, c = %f, minDCF = %f" %(K, C, c, DCF_min)," (noise version)\n")
                    # RBF Kernel
                    g_list = [1, 10]
                    for K in K_list:
                        for C in C_list:
                            for g in g_list:
                                DCF_min = minimum_detection_cost(svm_kernel_RBF(DTR_T, LTR_T, DTE, K, C, g=g), LTE, pi1, Cfn, Cfp)
                                print("SVM RBF Kernel: K = %f, C = %f, g = %f, minDCF = %f" %(K, C, g, DCF_min),"\n")
                                DCF_min = minimum_detection_cost(svm_kernel_RBF(DHTR_T, LHTR_T, DHTE, K, C, g=g), LHTE, pi1, Cfn, Cfp)
                                print("SVM RBF Kernel: K = %f, C = %f, g = %f, minDCF = %f" %(K, C, g, DCF_min)," (noise version)\n")
                ...
                if FLAG_GMM:
                    ...
                    for version in versions:
                        for M in M_list:
                            DCF_min = minimum_detection_cost(GMM_classifier(DTR_T, LTR_T, DTE, M, psi, version=version), LTE, pi1, Cfn, Cfp)
                            print("GMM: version = %s, M = %d, psi = %f, minDCF = %f" % (version, M, psi, DCF_min), "\n")
                            DCF_min = minimum_detection_cost(GMM_classifier(DHTR_T, LHTR_T, DHTE, M, psi, version=version), LHTE, pi1, Cfn, Cfp)
                            print("GMM: version = %s, M = %d, psi = %f, minDCF = %f" % (version, M, psi, DCF_min), "(noise version)\n")
        ...
        if FLAG_SINGLEFOLD:
            ...
            for app in applications:
                ...
                if FLAG_GAUSSIANS:
                    ...
                    generative_models = [("MVG", multivariate_gaussian_classifier2), ("Naive", naive_bayes_gaussian_classifier), ("Tied", tied_covariance_gaussian_classifier)]
                    for name, algo in generative_models:
                        DCF_min = minimum_detection_cost(algo(DTR, LTR, DTE), LTE, pi1, Cfn, Cfp)
                        print("%s: minDCF = %f" %(name,DCF_min),"\n") 
                        DCF_min = minimum_detection_cost(algo(DHTR, LHTR, DHTE), LHTE, pi1, Cfn, Cfp)
                        print("%s: minDCF = %f" %(name,DCF_min)," (noise version)\n") 
                    print("")
                ...
                if FLAG_SVM:
                    ...
                    for K in K_list:
                        for C in C_list:
                            DCF_min = minimum_detection_cost(svm_linear(DTR, LTR, DTE, K, C), LTE, pi1, Cfn, Cfp)
                            print("SVM Linear: K = %f, C = %f, minDCF = %f" %(K, C, DCF_min),"\n")
                            DCF_min = minimum_detection_cost(svm_linear(DHTR, LHTR, DHTE, K, C), LHTE, pi1, Cfn, Cfp)
                            print("SVM Linear: K = %f, C = %f, minDCF = %f" %(K, C, DCF_min)," (noise version)\n")
                    # Polynomial Kernel
                    c_list = [0,1]
                    for K in K_list:
                        for C in C_list:
                            for c in c_list:
                                DCF_min = minimum_detection_cost(svm_kernel_polynomial(DTR, LTR, DTE, K, C, d=2, c=c), LTE, pi1, Cfn, Cfp)
                                print("SVM Polynomial Kernel: K = %f, C = %f, d = 2, c = %f, minDCF = %f" %(K, C, c, DCF_min),"\n")
                                DCF_min = minimum_detection_cost(svm_kernel_polynomial(DHTR, LHTR, DHTE, K, C, d=2, c=c), LHTE, pi1, Cfn, Cfp)
                                print("SVM Polynomial Kernel: K = %f, C = %f, d = 2, c = %f, minDCF = %f" %(K, C, c, DCF_min)," (noise version)\n")
                    # RBF Kernel
                    g_list = [1, 10]
                    for K in K_list:
                        for C in C_list:
                            for g in g_list:
                                DCF_min = minimum_detection_cost(svm_kernel_RBF(DTR, LTR, DTE, K, C, g=g), LTE, pi1, Cfn, Cfp)
                                print("SVM RBF Kernel: K = %f, C = %f, g = %f, minDCF = %f" %(K, C, g, DCF_min),"\n")
                                DCF_min = minimum_detection_cost(svm_kernel_RBF(DHTR, LHTR, DHTE, K, C, g=g), LHTE, pi1, Cfn, Cfp)
                                print("SVM RBF Kernel: K = %f, C = %f, g = %f, minDCF = %f" %(K, C, g, DCF_min)," (noise version)\n")
                ...
                if FLAG_GMM:
                    ...
                    for version in versions:
                        for M in M_list:
                            DCF_min = minimum_detection_cost(GMM_classifier(DTR, LTR, DTE, M, psi, version=version), LTE, pi1, Cfn, Cfp)
                            print("GMM: version = %s, M = %d, psi = %f, minDCF = %f" % (version, M, psi, DCF_min), "\n")
